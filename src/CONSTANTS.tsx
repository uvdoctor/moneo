export const ROUTES = {
  HOME: "/",
  GET: "/get",
  SET: "/set",
  GROW: "/grow",
  CALCULATE: "/calculate",
  FI: "/fi",
  BR: "/buy-or-rent",
  EDUCATION: "/education-loan",
  LOAN: "/loan",
  TRUE_COST: "/true-cost",
  ABOUT: "/about",
  CONTACT_US: "/contact-us",
  FEATURES: "/features",
  SETTINGS: "/settings",
  TC: "/policies/terms-and-conditions",
  SECURITY: "/policies/security",
  PRIVACY: "/policies/privacy",
  LOOKUP: "/lookup",
  OVERVIEW: "/overview",
};

export const CALC_NAMES = {
  BR: "Buy v/s Rent & Invest",
  FI: "Financial Independence (FI)",
  LOAN: "Mortgage / Auto Loan",
  EDU_LOAN: "Education Loan",
  DR: "Debt Repayment",
  TC: "True Cost",
  IA: "Investment Analyzer",
};

export const ASSET_CATEGORIES = {
  STOCKS: "Stocks",
  BONDS: "Bonds",
  ALTERNATIVE: "Alternative",
};

export const LOCAL_INS_DATA_KEY = "insData";
export const LOCAL_NPS_DATA_KEY = "npsData";
export const LOCAL_FUN_DATA_KEY = "funData";
export const LOCAL_INDEX_PERF_KEY = "indexperfData";
export const LOCAL_INS_PERF_KEY = "insperfData";
export const LOCAL_CRYPTOLIST_DATA_KEY = "cryptolistData";
export const LOCAL_EXCHG_DATA_KEY = "listInExchgPrices";
export const LOCAL_DATA_TTL = { TTL: 86400000 }; //1 day

export const COLORS = {
  GREEN: "#499824",
  RED: "#FF0000",
  BLUE: "#3d86ce",
  DARK_BLUE: "#2c5282",
  DISABLED: "#cbd5e0",
  DEFAULT: "#4a5568",
  ORANGE: "#dd6b20",
  LIGHT_GRAY: "#edf2f7",
  LIGHT_GREEN: "#F0FFF4",
  SILVER: "#F1EDED",
  WHITE: "#ffffff",
};
