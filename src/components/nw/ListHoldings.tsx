...
export default function ListHoldings({
  ...
  const { selectedMembers, selectedCurrency, childTab, allFamily }: any =
  ...
  const expandedRow = (i: number) => {
    return (
      <Row
                ...
                <Row gutter={[10, 0]}>
                  <Col>
                    <Select
                      ...
                      <Option value={0}>Paid out</Option>
                      <OptGroup label="Accumulates every">
        ...
        {childTab === LENT && data[i].subt === NATIONAL_SAVINGS_CERTIFICATE && (
          <Col xs={24} sm={12} md={8}>
            <Row gutter={[10, 0]}>
              <Col>{"Duration"}</Col>
              <Col>
