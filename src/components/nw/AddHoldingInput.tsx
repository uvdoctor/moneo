...
export default function AddHoldingInput({
  ...
  const { allFamily, childTab, selectedMembers, selectedCurrency }: any =
  ...
  const changeStartMonth = (val: number) => {
    setSm(val);
    let rec = getNewRec();
    hasOnlyEnddate(childTab, subCat)
      ? category === "H"
        ? (rec.em = 0)
        : (rec.em = val)
      : (rec.sm = val);
              ...
              <Row gutter={[10, 0]}>
                <Col>
                  <DateInput
                    ...
                    endMonthHandler={
                      isRangePicker(childTab, category, subCat)
                        ? changeEndMonth
                        : undefined
                    }
                    endYearHandler={
