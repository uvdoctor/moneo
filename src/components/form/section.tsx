...
export default function Section(props: SectionProps) {
  const [showSection, setShowSection] = useState<boolean>(
    props.showOnLoad ? props.showOnLoad : true
  );

  return (
    <div
      className="ml-1 mr-1 mt-2 mb-4 w-full max-w-sm rounded overflow-hidden 
    ...
    >
      ...
      <div className="w-full bg-black">
        <label className="text-white py-1 pl-2 pr-1">{props.title}</label>
      </div>
      {props.toggle && showSection && (
          ...
          {props.manualMode && props.manualMode > 0 ? (
            ...
            <Fragment>
              <div className="pt-2 flex flex-wrap justify-around items-start w-full">
                ...
                {props.right && (
                  <div className="mt-2 md:mt-0">{props.right}</div>
                )}
              </div>
              {props.bottom && (
