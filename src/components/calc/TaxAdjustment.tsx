...
export default function TaxAdjustment() {
  ...
  return (
    <>
      <Section
        title={`Claim Tax ${
          isTaxCreditEligible(goal.type) ? "Credit" : "Deduction"
        }`}>
        ...
        {(isTaxCreditEligible(goal.type) || taxRate) &&
          goal.type !== GoalType.E && (
            <NumberInput
              info={`Maximum yearly ${
                isTaxCreditEligible(goal.type) ? "credit" : "deduction"
              } allowed. Enter zero in case there is no maximum deduction limit.`}
              pre={`Max yearly ${
                loanBorrowAmt && manualMode < 1 ? "principal" : ""
              } ${isTaxCreditEligible(goal.type) ? "credit" : "deduction"}`}
              currency={currency}
        ...
        {totalPTaxBenefit && (
          <ItemDisplay
            label={`Total ${
              loanBorrowAmt && manualMode < 1 ? "principal related" : ""
            } tax benefit`}
