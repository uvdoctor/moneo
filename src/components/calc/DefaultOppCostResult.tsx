  ...
  const calculateOppCost = (yearsNum: number) => {
    ...
    cfs.forEach((cf: number, index: number) => {
      if (sellAfter) {
        ...
        if (index === (startMonth > 1 ? sellAfter : sellAfter - 1))
          cf -= sellPrice;
      }
      oppCost += cf;
      let yearlyFactor = 1;
      if (index === 0 && startMonth > 1)
        yearlyFactor = (12 - (startMonth - 1)) / 12;
      if (index === cfs.length - 1 && startMonth > 1)
        yearlyFactor = (startMonth - 1) / 12;
      oppCost *=
        1 +
        ((isPublicCalc ? dr : discountRates[startIndex + index]) *
          yearlyFactor) /
          100;
      if (sellAfter && index === (startMonth > 1 ? sellAfter : sellAfter - 1))
        oppCost += sellPrice;
    });
    if (!isPublicCalc) {
