...
export default function StepThree(props: StepThreeProps) {
	...
	const handleFormChange = () =>
		props.setDisable(form.getFieldError('terms').length > 0 || !form.isFieldTouched('terms'));

	return (
		<Fragment>
			<Row>
				<Col span={24}>{props.error ? <Alert type="error" message={props.error} /> : null}</Col>
			</Row>
			<Row gutter={[ 0, 20 ]}>
				...
				<Col span={24}>
					<Checkbox
						defaultChecked={true}
						onChange={(e) => (e.target.checked ? props.setNotify(true) : props.setNotify(false))}
					>
