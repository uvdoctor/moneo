...
export default function Goal({ goal, summary, deleteCallback, modalCallback, wipGoalCallback, addCallback }: GoalProps) {
    ...
    const [loanPer, setLoanPer] = useState<number>(id && goal && goal.emi && goal.emi.per ? goal.emi.per : 0)
    const [currentPrice, setCurrentPrice] = useState<number>(id && goal && goal.cp ? goal.cp : 0)
    const [currency, setCurrency] = useState<string>(id && goal && goal.ccy ? goal.ccy : "USD")
    const fxRate = 1.0
    const [criticality, setCriticality] = useState<APIt.LMH>(id && goal && goal.imp ? goal.imp : APIt.LMH.M)
    const [ra, setRA] = useState<APIt.LMH>(APIt.LMH.L)
    const [manualMode, setManualMode] = useState<number>(0)
    const [name, setName] = useState<string>(id && goal && goal.name ? goal.name : "")
    const [emi, setEmi] = useState<number>(0)
    const [loanMonths, setLoanMonths] = useState<number>(id && goal && goal.emi && goal.emi.dur ? goal.emi.dur : 6)
    const [loanIntRate, setLoanIntRate] = useState<number>(id && goal && goal.emi && goal.emi.rate ? goal.emi.rate : 4)
    const [priceChgRate, setPriceChgRate] = useState<number>(id && goal && goal.chg ? goal.chg : 3)
    const [amCostPer, setAMCostPer] = useState<number>(id && goal && goal.amper ? goal.amper : 2)
    const [amStartYear, setAMStartYear] = useState<number>(id && goal && goal.amsy ? goal.amsy : startYear)
    const [oppDR, setOppDR] = useState<number>(id && goal && goal.dr ? goal.dr : 6)
