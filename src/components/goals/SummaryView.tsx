...
import { getImpLevels, getDefaultIconForGoalType, goalImgStorage } from './goalutils';
...
export default function SummaryView() {
	...
	const [
		goalImg,
		setGoalImg
	] = useState<string | null>(goalImgUrl);
	const [
		goalSY,
		setGoalSY
	] = useState<number>(goal.sy);
	const getImpColor = (imp: LMH) => (imp === LMH.H ? COLORS.BLUE : imp === LMH.M ? COLORS.ORANGE : COLORS.GREEN);
	const [
		impColor,
		setImpColor
	] = useState<string>(getImpColor(impLevel as LMH));
	const impLevels: any = getImpLevels();
	const { confirm } = Modal;
	const [
		lastUpdated,
		setLastUpdated
	] = useState<string>(getDaysDiff(goal.updatedAt));
						...
						<Row align="middle">
							<Col>
								<Avatar
									...
									style={{ backgroundColor: COLORS.WHITE, color: COLORS.DEFAULT }}
								/>
							</Col>
							<Col>
								<hgroup>
									...
									<h4>
										Costs ~ &nbsp;
										{toHumanFriendlyCurrency(Math.abs(totalCost), currency)}
									</h4>
								</hgroup>
							</Col>
						</Row>
