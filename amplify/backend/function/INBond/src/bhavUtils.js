...
const extractDataFromCSV = async (fileName, typeExchg, codes, schema, isinMap, table) => {
	const end = new Promise((resolve, reject) => {
		...
		fs
			...
			.on('data', (record) => {
				...
				const updateSchema = calcSchema(record, codes, schema, typeExchg, isinMap, table);
			...
			.on('error', (err) => {
				cleanDirectory(tempDir, `Unable to read ${typeExchg} csv file, ${err.message}`);
